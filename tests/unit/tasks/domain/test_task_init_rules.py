...
from app.modules.tasks.domain.task_errors import InvalidTaskStatusTransitionError, InvalidTaskDueDateError
...
def test_init_raises_when_completed_task_is_blocked() -> None:
    ...
    with pytest.raises(InvalidTaskStatusTransitionError, match="cannot be blocked in the target state"):
        build_task(status=TaskStatus.COMPLETED, due_date=future_date(), is_blocked=True)


def test_init_raises_when_cancelled_task_is_blocked() -> None:
    # Arrange / Act / Assert
    with pytest.raises(InvalidTaskStatusTransitionError, match="cannot be blocked in the target state"):
        build_task(status=TaskStatus.CANCELLED, due_date=future_date(), is_blocked=True)
