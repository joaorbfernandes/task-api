...
from app.modules.tasks.domain.task_errors import InvalidTaskStatusTransitionError, InvalidTaskDueDateError, TaskNotEditableError
...
def test_change_status_allows_pending_to_in_progress_when_due_date_exists() -> None:
    # Arrange
    task = build_task(status=TaskStatus.PENDING, due_date=future_date())
...
def test_change_status_allows_blocked_pending_to_in_progress_and_clears_blocked() -> None:
    # Arrange
    task = build_task(status=TaskStatus.PENDING, due_date=future_date(), is_blocked=True)
...
def test_change_status_allows_blocked_in_progress_to_cancelled_and_clears_blocked() -> None:
    # Arrange
    task = build_task(status=TaskStatus.IN_PROGRESS, due_date=future_date(), is_blocked=True)
