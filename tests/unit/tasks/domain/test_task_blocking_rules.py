...
from app.modules.tasks.domain.task_errors import InvalidTaskStatusTransitionError, TaskNotEditableError
...
def test_block_sets_task_as_blocked_when_status_is_in_progress() -> None:
    # Arrange
    task = build_task(status=TaskStatus.IN_PROGRESS, due_date=future_date(), is_blocked=False)
