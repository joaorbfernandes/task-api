...
from app.modules.tasks.domain.task_errors import TaskNotEditableError, InvalidTaskStatusTransitionError, InvalidTaskDueDateError
...
def test_update_allows_in_progress_to_keep_same_status_and_change_other_fields() -> None:
    ...
    assert task.status == TaskStatus.IN_PROGRESS
    assert task.description == "new description"
    assert task.due_date == future_date(5)
    assert task.is_blocked is True
